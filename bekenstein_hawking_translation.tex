...
\begin{theorem}[A/F/R partition, {\cite[BackboneDichotomy.lean]{SilvaAlvarado26}}]
\label{thm:afr}
...
\begin{theorem}[Involution identities, {\cite[BackboneDichotomy.lean]{SilvaAlvarado26}}]
...
\begin{theorem}[{\cite[HarmonicCentroid.lean]{SilvaAlvarado26}}]
\label{thm:wtb}
For any finite poset $P$ and incomparable pair $a \parallel b$:
...
\begin{remark}[What remains for full perturbative matching]
The tree-level identification (structure constants, vertex
factors) is established.  The 1-loop identification
(the beta function $\beta_0 = -11 N_c / (48\pi^2)$)
requires computing the discrete 1-loop self-energy
correction in the Kuratowski vacuum.  This is a specific,
computable quantity: the sum over all $\Kthree$ subgraphs
at Hasse distance~1 from a given vertex, weighted by
their $\tau$ contributions.  The computation is deferred
to a companion paper; the prediction is that the discrete
beta function matches the continuum value at $N_c = 3$.
\end{remark}
